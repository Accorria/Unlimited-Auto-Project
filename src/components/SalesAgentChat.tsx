...
export default function SalesAgentChat({ 
    ...
    if (isOpen && displayMessages.length === 0 && !displayLoading && showOptions) {
      ...
      const greetingTimer = setTimeout(() => {
        const greetingMessage = {
          ...
          role: 'assistant',
          content: vehicleName 
            ? `Hey, interested in the ${vehicleName}? Would you like to set up an appointment for a test drive? Or do you want to fill out a pre-approval? Or would you like a call?`
            : `Hey, interested in this? Would you like to set up an appointment for a test drive? Or do you want to fill out a pre-approval? Or would you like a call?`
        }
  ...
  const handleCallFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!callFormData.name || !callFormData.phone) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Show success message and phone number
        const callMessage = {
          id: 'call-' + Date.now(),
          role: 'assistant',
          content: `Thanks ${callFormData.name}! We've got your info. Give us a call at (313) 766-4475. We're here to help!`
        }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl h-[90vh] sm:h-[80vh] max-h-[700px] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
          {displayMessages.length === 0 && !displayLoading && !showQuickForm && !showOptions && (
          ...
          {/* Options Buttons */}
          {showOptions && displayMessages.length > 0 && !showAppointmentScheduler && !showCallForm && (
            <div className="space-y-3">
              <button
                onClick={() => handleOptionClick('test_drive')}
                className="w-full bg-blue-600 text-white py-3 px-4 rounded-lg hover:bg-blue-700 transition-colors font-semibold text-sm"
              >
                🚗 Schedule Test Drive
              </button>
              <button
              ...
              <button
                ...
                className="w-full bg-gray-600 text-white py-3 px-4 rounded-lg hover:bg-gray-700 transition-colors font-semibold text-sm"
              >
                📞 Call Us
              </button>
            </div>
          ...
          {showCallForm && (
            <div className="bg-white border-2 border-gray-500 rounded-lg p-4 space-y-3">
              <div className="flex justify-between items-center">
                <h4 className="font-bold text-gray-900">Request a Call</h4>
                <button
                  onClick={() => {
                    setShowCallForm(false)
                    setShowOptions(true)
